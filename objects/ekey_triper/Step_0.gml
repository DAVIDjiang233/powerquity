y=scr_yset(endtime);
x=xset(key1,endtime);
if(global.textlist[20+key1]!=keyamo||global.textlist[21+key1]!=keyamo2||global.textlist[22+key1]!=keyamo3) exit;
...
||global.pressing[global.pressreal[key1]]==3
||global.pressing[global.pressreal[key1+1]]==3){
	if(exjudge==0){
		global.textlist[2]++;
		global.textlist[3]++;
		global.textlist[4]++;
		...
		{
			stime : global.playtime,
			spr : global.playskin[scr_judge(endtime)*2+243+key1],
			sfps : global.skinnumber[23]
		}
		);
		instance_create_depth((global.playtime-endtime)/global.judgesize,global.settings[9],-5000,showhit);
	}
	global.textlist[20+key1]=keyamo+0.3;
	global.textlist[21+key1]=keyamo2+0.3;
	global.textlist[22+key1]=keyamo3+0.3;
	instance_destroy();
}