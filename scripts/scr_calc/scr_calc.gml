		...
		if(_rpn[_i]=="x"){
			...
			if(array_last(_cal)<array_length(global.textlist)&&array_last(_cal)>=0){
				if(typeof(global.textlist[array_last(_cal)])=="string"){
					_cal[array_length(_cal)-1]=string_length(global.textlist[array_last(_cal)]);
				}
			...
			if(_cal[array_length(_cal)-1]<array_length(global.currenttext)
			&&global.currenttext[_cal[array_length(_cal)-1]]!=0){
				var _font=draw_get_font();
				draw_set_font(global.fontall[global.currenttext[_cal[array_length(_cal)-1]][9]]);
				_cal[array_length(_cal)-1]=
				string_width(string_ext(global.currenttext[_cal[array_length(_cal)-1]][4],global.textlist))
				/font_get_size(global.fontall[global.currenttext[_cal[array_length(_cal)-1]][9]])*72;
				_i++;
				draw_set_font(_font);
			}
			else return error_result;
